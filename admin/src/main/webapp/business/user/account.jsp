...
<script type="text/javascript">
      ...
      //设置翻页工具栏中的首页、尾页、上一页、下一页
		...
		$("#saveBtn").click(function(){
            ...
            //过滤 回车、空格等
             ...
             // 修改 
             $.ajax({
             	url:serviceurl.baseurl+"/<%=busType%>/account/save",
                data:"{\"account\":\""+msg+"\",\"operator\":\""+operator+"\"}",
                dataType:"json",
                 beforeSend:function(){
                     //执行提交前的提交信息验证
                    return validateForm();
    ...
    function  getPageData(pageNo){
        $.ajax({
            ...
            url:serviceurl.baseurl+"/<%=busType%>/account/get/page",
            type: 'POST',
            data: {'account':$('#searchaccount').val(),'status':$('#searchstatus').val(),'pageNo':pageNo,'operator':'<%=userid%>'},
            dataType:"json",
            success:dataFill
        });
    }
    ...
    function dataFill(data) {
        ...
        if(resultObject.totalPages > 0) {
            for(var i=0; i<resultObject.result.length;i++) {
                ...
                datatr+="<td>"+resultData.statusString+"</td>";
								...
								<form name="form1" id="form1">
									...
									<div class="box-body">
										...
										<div class="form-group">
											<label>添写帐号</label>
											<textarea cols="20" rows="20" id="account"
												style="width: 100%;" name="account" class="form-control"
												placeholder="(输入多个帐号以横线隔开,不允许有回车换行。形如：123123-222323-3434343)" ></textarea>
										</div>
