...
public class SimpleHibernateDao<T, PK extends Serializable> {
     ...
     * @throws Exception
     */
    public void batchUpdate(final List<String> entityList){
	Session session = sessionFactory.openSession();
	Transaction tx = session.beginTransaction();

	try {
	    int i = 0;
	    for (String sql : entityList) {
		i++;
		...
		logger.debug("update entity: {}", sql);
	    }
	    tx.commit();
	} catch (Exception e) {
	    tx.rollback(); // 出错将回滚
    ...
    /**
     * 对原生SQL查询执行与参数列表创建Query对象.
     */
    public SQLQuery createSQLQuery(final String sqlString) {
